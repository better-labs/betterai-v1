...
export function PredictionGeneratorV2({ marketId }: PredictionGeneratorV2Props) {
  ...
  // Calculate total cost including research source
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Cost Summary with Research Breakdown */}
          {(selectedModels.length > 0 || researchCost > 0) && (
            <div className={components.costBreakdown.container}>
              {selectedModels.length > 0 && (
                <div className={components.costBreakdown.row}>
                  <span className={components.costBreakdown.label}>AI Models:</span>
                  <span className={components.costBreakdown.value}>{modelCost} credit{modelCost !== 1 ? 's' : ''}</span>
                </div>
              )}
              {researchCost > 0 && (
                <div className={components.costBreakdown.row}>
                  <span className={components.costBreakdown.label}>Research:</span>
          ...
          <div>
            ...
            >
              {isGenerating ? (
                ...
                `Generate Prediction${selectedModels.length > 1 ? 's' : ''} with Research`
              )}
            </Button>
          </div>
        </CardContent>
